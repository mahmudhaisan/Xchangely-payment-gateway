...
class Paymenttory_API {
     ...
     * @param Paymenttory_Settings $settings Plugin settings.
     * @throws Exception
     */
    public static function get_pay_url_for_order(WC_Order $order, WC_Customer $customer, Paymenttory_Settings $settings) {
        ...
        $subscription = false;
        // Check Order Contain Subscription
        // at first check wcs_order_contains_subscription function is available or not
        ...
        if($subscription){
            ...
            if (count($order_subscriptions) > 1) {
                wc_add_notice(__('Only one subscription may be purchased at a time through Paddle.', 'wp-paymenttory'), 'error');
               // return cart response error
                return [
                    'result' => 'failure',
                    'errors' => WC()->session->get('wc_notices', array())
                ];
            } elseif (count($order_subscriptions) == 1 && count($order->get_items()) > 1) {
                wc_add_notice(__('Paddle now allowing to purchase a subscriptions and one-time charges product at the same time. Remove other products.', 'wp-paymenttory'), 'error');
                return [
                    'result' => 'failure',
                    'errors' => WC()->session->get('wc_notices', array())
                ];
            ...
            require_once __DIR__ . '/subscription/Paddle_Subscription.php';
            $subscription_data = (new Paddle_Subscription)->get_data($order);

            $product_id = (new Paddle_Subscription)->get_paddle_subs_plan($order, $settings);;

            $custom_message = sprintf("Initial Amount: %s | Recurring Amount: %s | Trial days: %s", $subscription_data['initial_amount'], $subscription_data['recurring_amount'], $subscription_data['trial_days'] ?? 0);

            // Overwrite default title.
            $subscription_payload = [
                'vendor_id'           => $settings->get('paddle_vendor_id'),
                'vendor_auth_code'    => $settings->get('paddle_api_key'),
                'title'             => $subscription_data['plan_name'],
                'custom_message'    => mb_strimwidth($custom_message, 0, 252, '...'),
                'customer_email'    => $order->get_billing_email(),
                'passthrough'       => $order->get_id(),
                'product_id'        => $product_id,
                'prices'            => [
                    $order->get_currency() .':'. $subscription_data['initial_amount']
                ],
                'recurring_prices'  => [
                    $order->get_currency() .':'. $subscription_data['recurring_amount']
                ],
                'trial_days'        => $subscription_data['trial_period'] ?? 0,
                'quantity'          => 1,
                'quantity_variable' => 0,
                // 'discountable'      => 0,
        ...
        $payload = $subscription ? $subscription_payload : $data;

		// Get pay link from Paddle API
		$post_url_root = Paymenttory_Settings::instance()->getPaddleRootURL();
		$post_url = $post_url_root . Paymenttory_Settings::API_GENERATE_PAY_LINK_URL;
		...
		if (is_wp_error($api_call_response)) {
			...
			wc_add_notice( __( 'Something went wrong getting checkout url. Unable to get API response.', 'wp-paymenttory' ), 'error');
			error_log(__( 'Paddle error. Unable to get API response. Method: ', 'wp-paymenttory' ) . __METHOD__ . __( ' Error message: ', 'wp-paymenttory' ) . $api_call_response->get_error_message());
			return json_encode(array(
				'result' => 'failure',
				'errors' => array(__( 'Something went wrong. Unable to get API response.', 'wp-paymenttory' ))
			));
		} else {
			...
			if ($api_response && $api_response->success === true) {
				...
				wc_add_notice(__( 'Something went wrong getting checkout url. Check if gateway is integrated.', 'wp-paymenttory' ), 'error');
				if (is_object($api_response)) {
					error_log(__( 'Paddle error. Error response from API. Method: ', 'wp-paymenttory' ) . __METHOD__ . __( ' Errors: ', 'wp-paymenttory' ) . print_r($api_response->error, true));
				} else {
					error_log(__( 'Paddle error. Error response from API. Method: ', 'wp-paymenttory' ) . __METHOD__ . __( ' Response: ', 'wp-paymenttory' ) . print_r($api_call_response, true));
				}
				return json_encode(array(
					'result' => 'failure',
					'errors' => array(__( 'Something went wrong. Check if Paddle account is properly integrated.', 'wp-paymenttory' ))
				));
			}
		}
	}
    ...
    private static function get_webhook_url($order_id) {
	    ...
	    return get_bloginfo('url') . '/index.php/wc-api/wp_paymenttory?order_id=' . $order_id;
	}
	...
	public static function check_webhook_signature() {
		...
		$vendor_public_key = Paymenttory_Settings::instance()->getPaddleVendorKey();
		if (empty($vendor_public_key)) {
            error_log(__( 'Paddle error. Unable to verify webhook callback - vendor_public_key is not set.', 'wp-paymenttory' ));
			return -1;
		}
