...
class Xchangely_Checkout {
    ...
    /**
     * Xchangely_Checkout Constructor.
     */
...
//    
        ...
        $paddle_data = array(
            ...
            'is_sandbox' => $this->settings->get('enabled_sandbox') == 'yes',
            'unable_process_order' => __( "We were unable to process your order, please try again in a few minutes.", 'wp-paymenttory' )
        );
    ...
    public function on_ajax_process_checkout_pay() {
        if (!WC()->session->order_awaiting_payment) {
            wc_add_notice(__( 'We were unable to process your order, please try again.', 'wp-paymenttory' ), 'error');
            ob_start();
            wc_print_notices();
            ...
            echo json_encode(array(
                'result' => 'failure',
                'messages' => $messages,
                'errors' => array(__( 'We were unable to process your order, please try again.', 'wp-paymenttory' ))
            ));
            exit;
        }
     ...
     */
    private function get_ajax_endpoint_path(string $endpoint) {
